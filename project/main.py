...
class GameApp:
    ...
    def draw(self):
        ...
        if GAME_SETTINGS.get("show_fps"):
            fps_text = self.ui.font_xs.render(f"FPS {int(self.clock.get_fps())}", True, GREEN)
            self.game_surface.blit(fps_text, (WIDTH - 52, HEIGHT - 12))
        ...
        if event.key in (pygame.K_UP, pygame.K_w, 1094, 1062):
            ...
            if self.menu_page == "main":
                ...
                if self.menu_index == 0:
                    ...
                    GAME_SETTINGS["show_fps"] = not GAME_SETTINGS.get("show_fps", False)
                    save_settings(GAME_SETTINGS)
