...
class UI:
    def __init__(self):
        self.font_xs = _load_font("PressStart2P-Regular.ttf", 7)
        self.font_sm = _load_font("PressStart2P-Regular.ttf", 8)
        self.font_md = _load_font("RussoOne-Regular.ttf", 14)
        self.font_lg = _load_font("RussoOne-Regular.ttf", 22)
        self.font_title = _load_font("RussoOne-Regular.ttf", 26)
    ...
    def draw_main_menu(
        ...
        if current_page == "main":
            title_y = 28
            self._draw_text_shadow(screen, self.font_title, "МИР", PLAYER_COLOR, (WIDTH // 2 - 42, title_y))
            ...
            show_fps = "ВКЛ" if GAME_SETTINGS.get("show_fps") else "ВЫКЛ"
    ...
    def draw_hud(self, screen, player, dungeon):
        ...
        for i in range(player.max_hp):
            ...
            screen.blit(icon, (6 + i * 16, 4))

        screen.blit(self._coin_icon, (WIDTH // 2 - 36, 6))
        coin_text = self.font_sm.render(str(player.coins), True, COIN_COLOR)
        screen.blit(coin_text, (WIDTH // 2 - 20, 8))

        screen.blit(self._star_icon, (WIDTH // 2 + 16, 6))
        score_text = self.font_sm.render(str(player.score), True, WHITE)
        screen.blit(score_text, (WIDTH // 2 + 32, 8))
        ...
        room_label = room_names.get(pos, f"{pos[0]},{pos[1]}")
        room_text = self.font_xs.render(room_label, True, (140, 145, 160))
        screen.blit(room_text, (WIDTH - room_text.get_width() - 6, 22))

        if dungeon.doors_locked:
            battle = self.font_md.render("БОЙ!", True, RED)
            pulse = int(128 + 127 * math.sin(self._menu_tick * 0.15))
            battle.set_alpha(pulse)
            screen.blit(battle, (WIDTH // 2 - battle.get_width() // 2, 20))
